...
struct DataType
{
    ...
    bool operator==(const DataType& other) const { return mInt == other.mInt && mBool == other.mBool; }
};
...
{
public:
    MMapWriter(const std::string& filename)
    :
        mFilename{ filename }
    {
        mFileDescriptor = shm_open(mFilename.c_str(), O_CREAT | O_RDWR, S_IRUSR | S_IWUSR);
        ...
        mMappedMemory = mmap(nullptr, sizeof(DataType), PROT_READ | PROT_WRITE, MAP_SHARED, mFileDescriptor, 0);
