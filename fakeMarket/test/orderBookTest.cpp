...
TEST_F(OrderBookTestFixture, TestFAK)
{
    ...
    EXPECT_EQ(expectedTob, ob.GetTopOfBook());
    EXPECT_FALSE(
        CheckQuoteExists(Common::MakeHashId(client1, pId, QuoteId{1}))
    ); 
    EXPECT_FALSE(
        CheckQuoteExists(Common::MakeHashId(client2, pId, QuoteId{1}))
    ); 
    EXPECT_FALSE(
        CheckQuoteExists(Common::MakeHashId(client1, pId, QuoteId{2}))
    ); 
    ...
    EXPECT_EQ(expectedTob, ob.GetTopOfBook());
    EXPECT_FALSE(
        CheckQuoteExists(Common::MakeHashId(client1, pId, QuoteId{3}))
    ); 
