...
#include "filters/deterministicEKF.hpp"

namespace Filters {

template <typename Traits, int NumSamples>
DeterministicEKFImpl<Traits, NumSamples>::DeterministicEKFImpl(
...
DeterministicEKFImpl<Traits, NumSamples>::DeterministicEKFImpl(
    ...
    : DeterministicEKFImpl{stateEstimate,
                        covEstimate,
...
template <typename Traits, int NumSamples>
void DeterministicEKFImpl<Traits, NumSamples>::Update(const ObsVector& obs) {
...
template <typename Traits, int NumSamples>
const DeterministicEKFImpl<Traits, NumSamples>::HiddenVector&
DeterministicEKFImpl<Traits, NumSamples>::Predict() {
...
template <typename Traits, int NumSamples>
void DeterministicEKFImpl<Traits, NumSamples>::InitialiseSamples(
    const HiddenVector& stateEstimate, const HiddenMatrix& covEstimate) {
...
template <typename Traits, int NumSamples>
void DeterministicEKFImpl<Traits, NumSamples>::UpdateSamples(const ObsVector& obs,
                                                          const ObsMatrixT& K) {
    for (unsigned i = 0; i < NumSamples; ++i) {
