...
class ParticleFilter {
 public:
  ParticleFilter(
      unsigned numberOfParticles, unsigned numberOfHiddenVariables,
      std::function<Eigen::MatrixXd()> const& initialSampler,
      std::function<Eigen::MatrixXd(const Eigen::MatrixXd&)> const&
          transitionSampler,
      std::function<double(const Eigen::MatrixXd&,
                           const Eigen::MatrixXd&)> const& probabilityObserved)
 ...
 private:
  ...
  std::function<Eigen::MatrixXd(const Eigen::MatrixXd&)> const
      mTransitionSampler;
  std::function<double(const Eigen::MatrixXd&, const Eigen::MatrixXd&)> const
      mProbabilityObserved;
};
