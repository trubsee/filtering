...
LinearGaussian::LinearGaussian(const Eigen::MatrixXd& coefficients, const Eigen::MatrixXd& covarianceMatrix)
:
    ...
    mCoef{coefficients}
{
    ...
    mMaxProb = 1 / std::sqrt(std::pow(2 * PI, mOutputs) * covarianceMatrix.determinant());
}
...
double LinearGaussian::Probability(const Eigen::VectorXd& output, const Eigen::VectorXd& input) const
{
    ASSERT(output.size() == mOutputs);
    ASSERT(input.size() == mInputs);
    const auto relative = (output - mCoef * input).transpose() * mInverseNoise * (output - mCoef * input);
