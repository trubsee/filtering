...
public:
    ...
    unsigned CreateTimer(const Timer::ClockTime::duration& interval, Timer::CallbackFunc callback)
    {
        ...
        auto it = std::upper_bound(mTimers.begin(), mTimers.end(), timer, std::greater<Timer>());
    ...
    void Update()
    {
        ...
        while (mTimers[mTimers.size()-1].CheckFire(Clock::now()))
        {
            ...
            mTimers.pop_back();
            const auto it = std::upper_bound(mTimers.begin(), mTimers.end(), timer, std::greater<Timer>());
