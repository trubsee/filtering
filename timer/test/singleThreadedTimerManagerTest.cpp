...
TEST(SingleThreadedTimerManagerTest, Update) {
  ...
  int counter1{0};
  auto id1 = timerManager.CreateTimer(std::chrono::seconds(3),
                                      [&counter1]() { ++counter1; });

  int counter2{0};
  auto id2 = timerManager.CreateTimer(std::chrono::seconds(2),
                                      [&counter2]() { ++counter2; });

  int counter3{0};
  auto id3 = timerManager.CreateTimer(std::chrono::seconds(5),
                                      [&counter3]() { ++counter3; });
